...
-- KSCM-1367
-- Add Open Document where Doctype = kuali.proposal.type.course.create and Route status = R permission.
INSERT INTO KRIM_PERM_T (ACTV_IND,DESC_TXT,NM,NMSPC_CD,OBJ_ID,PERM_ID,PERM_TMPL_ID,VER_NBR)
  VALUES ('Y','Allows users to open Create Course Proposals with a Route Status of Enroute.','Open Create Course Route Status R','KS-SYS',SYS_GUID(),KRIM_PERM_ID_S.NEXTVAL,
  (SELECT PERM_TMPL_ID FROM KRIM_PERM_TMPL_T WHERE NMSPC_CD='KS-SYS' AND NM='Open Document'),1)
/
...
-- Assign to Department Chair role
...
-- Assign to College Curriculum Manager role
-- Doctype = kuali.proposal.type.course.create and Route status = R permission.
INSERT INTO KRIM_ROLE_PERM_T (ACTV_IND,OBJ_ID,PERM_ID,ROLE_ID,ROLE_PERM_ID,VER_NBR)
